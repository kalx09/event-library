...
class EventDal:
    ...
    ADD_EVENT = "INSERT INTO event_info (event_noun) VALUES (%s);"

    @staticmethod
    def add_event(event_noun):
        """
        Add's event to db
        :param event_noun: Name of the unique event [String, Required]
        :return: void
        """
        db_obj.execute(EventDal.ADD_EVENT, (event_noun,))
    ...
    ADD_RULE_TO_EVENT = "INSERT into event_rule (event_rule_name, event_rule_verb, event_rule_no_of_attempts, " \
                        "event_rule_time_interval, event_noun_id) VALUES (%s, %s, %s, %s, %s);"

    @staticmethod
    def add_rule_to_event(event_rule, event_noun):
        """
        ...
        :return: void
        """
        event_result_set = db_obj.execute(EventDal.GET_EVENT_ID, (event_noun,))
        if event_result_set.has_row():
            ...
            db_obj.execute(EventDal.ADD_RULE_TO_EVENT, (event_rule["event_rule_name"],
                                                                     ...
                                                                     int(event_rule["event_rule_no_of_attempts"]), int(event_rule["event_rule_time_interval"]), event_noun_id, ))

    ####################################################################################################

    GET_EVENT_RULE_ID = "SELECT * FROM event_rule WHERE LOWER(event_rule_name) = LOWER(%s) " \
                        "and LOWER(event_rule_verb) = LOWER(%s)"
    ADD_EVENT_ATTEMPT = "INSERT into event_execute (event_rule_id) VALUES (%s);"

    @staticmethod
    def is_event_rule_in_db(event_rule_name, event_rule_verb):
        """
        ...
        """
        result_set = db_obj.execute(EventDal.GET_EVENT_RULE_ID, (event_rule_name, event_rule_verb))
        return True if result_set.has_row() else False

    @staticmethod
    def get_event_rule_info(event_rule_name, event_rule_verb):
        """
    ...
    @staticmethod
    def add_execution_attempt(event_rule_name, event_rule_verb):
        """
        Add's execution attempt/call to db
        :param event_rule_name: Name of the event rule name [String]
        :param event_rule_verb: Name of the event rule verb [String]
        :return: void
        """
        event_rule_result_set = db_obj.execute(EventDal.GET_EVENT_RULE_ID, (event_rule_name, event_rule_verb, ))
        if event_rule_result_set.has_row():
            event_rule_id = event_rule_result_set.get_rows()[0]["event_rule_id"]
            db_obj.execute(EventDal.ADD_EVENT_ATTEMPT, (event_rule_id, ))

    @staticmethod
    def get_event_rule_id(event_rule_name, event_rule_verb):
        """
    ...
    GET_ALL_EVENTS_WITHIN_TIME_INTERVAL = "SELECT * FROM event_execute WHERE event_rule_id = %s and " \
                                          "TIMESTAMP > NOW() - INTERVAL %s MINUTE"

    @staticmethod
    def get_no_of_eventcalls_within_time_interval(event_rule_name, event_rule_verb):
        """
        Retrieve no of calls made for a specific event rule
        :param event_rule_name: Name of the event rule to retrieve information for [String, Required]
        :param event_rule_verb: Name of the event verb [String, Required]
        ...
        if event_rule_result_set.has_row():
            ...
            event_rule_id = event_row_set[0]["event_rule_id"]
            ...
            result_set = db_obj.execute(EventDal.GET_ALL_EVENTS_WITHIN_TIME_INTERVAL,
                                        (event_rule_id, event_rule_time_interval))
            return result_set.row_count()

    ####################################################################################################

    DELETE_EXECUTED_EVENTS = "DELETE FROM event_execute WHERE event_rule_id = %s and " \
                             "TIMESTAMP < NOW() - INTERVAL %s MINUTE"

    @staticmethod
    def remove_stale_executed_event(event_rule_name, event_rule_verb):
        """
        Deletes stale event executed from db
        :param event_rule_name: Name of the event rule which needs to be deleted [String, Required]
        :param event_rule_verb: Name of the event verb [String, Required]
        :return: Count of the deleted executed events [Integer]
        """
        event_rule_result_set = db_obj.execute(EventDal.GET_EVENT_RULE_ID, (event_rule_name, event_rule_verb,))
        if event_rule_result_set.has_row():
            event_row_set = event_rule_result_set.get_rows()
            event_rule_id = event_row_set[0]["event_rule_id"]
            event_rule_time_interval = event_row_set[0]["event_rule_time_interval"]
            result_set = db_obj.execute(EventDal.DELETE_EXECUTED_EVENTS,
                                        (event_rule_id, event_rule_time_interval))
            return result_set.row_count()

    ####################################################################################################

    ADD_EVENT_DATA = "INSERT INTO event_data (event_noun_id, event_rule_id, event_data_json_rep) VALUES (%s, %s, %s)"
    GET_EVENT_DATA = "SELECT event_data_json_rep FROM event_data %s LIMIT %s OFFSET %s"

    @staticmethod
    def add_event_data(event_noun_id, event_rule_id, json_rep):
        """
        Add's event data to db
        :param event_noun_id: Event id of the event for which data is required to be stored [Integer, Required]
        :param event_rule_id: Event rule id of the event for which data is required to be stored [Integer, Required]
        :param json_rep: Event data [String, Required]
        :return: void
        """
        db_obj.execute(EventDal.ADD_EVENT_DATA, (event_noun_id, event_rule_id, json_rep))
    ...
    def get_event_data(page_number, no_of_items_per_page, event_noun=None, event_rule_name=None, event_rule_verb=None):
        ...
        :param event_rule_name: Name of the event rule [String, Optional]
        :param event_rule_verb: Name of the event verb [String, Optional]
